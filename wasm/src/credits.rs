...
pub async fn transfer(from: &str, to: &str, amount: u64) -> Result<String, JsValue> {
    ...
    header.insert(reqwest_wasm::header::ACCEPT, reqwest_wasm::header::HeaderValue::from_static("application/json"));
    ...
    let execution_transaction = client
        ...
        .headers(header)
        .json(&json!(ExecutionRequest::new(execution_transaction, to, program_id).unwrap()))
        .send()
        .await
        .unwrap()
...
mod tests {
    ...
    use aleo_account::Transaction;
    use wasm_bindgen_test::wasm_bindgen_test;

    #[wasm_bindgen_test]
    async fn test_transfer() {
        const FROM: &str = "APrivateKey1zkp6ka4UZu9JfMFDBqzKBffX7HQpYneKtzCTwnHb5GXVDpL";
        const TO: &str = "aleo1hf0jutqqeqv2nhazntuted4z99ax873lgfaw623ytqc68z72cqqqa9xeg4";
        const AMOUNT: u64 = 0;

        let transfer_result = transfer(FROM, TO, AMOUNT).await;

        assert!(transfer_result.is_ok());

        let transaction_id = transfer_result.unwrap();

        let res = reqwest_wasm::Client::new()
            .get(&format!("http://localhost/testnet3/transactions/{transaction_id}"))
            .send()
            .await
            .unwrap()
            .json::<Transaction>()
        ...
        assert_eq!(transaction.id().to_string(), transaction_id);
